...
# CHANGED: point to processed\balanced_segments (not data\processed\balanced_segments)
...
# ------------------ MAIN ------------------
def main():
    ap = argparse.ArgumentParser(description="ECAPA-TDNN speaker embedding extraction (train + test)")
    ap.add_argument("--data-root", default=str(DEFAULT_DATA_ROOT), help="Input data root containing train/test folders")
    ap.add_argument("--out-root",  default=str(DEFAULT_OUT_ROOT), help="Output folder root")
    ap.add_argument("--splits", nargs="+", default=["train", "test"], help="Which splits to process")
